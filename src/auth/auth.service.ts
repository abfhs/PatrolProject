...
export class AuthService {
     ...
     * 2) 로그인 할때는 BasicToken과 함꼐 요청을 보낸다.
     *  BasicToken은 '이메일:비밀번호'를 base64로 인코딩한 형태이다.
     *  예) {authorization: 'Basic {token}'}
     * 
     * 3) 아무나 접근 할 수 없는 정보 {private route}를 접근 할떄는
     *  accessToken을 Header에 추가해서 요청과 함꼐 보낸다.
     *  예) {authorization: 'Bearer {token}'}
     * 
     * 4) 토큰과 요청을 함꼐 받은 서버는 토큰 검증을 통해 현재 요청을 보낸 사용자가
     *  누구인지 알 수 있다.
     *  예를 들어서 현재 로그인한 사용자가 작성한 포스트만 가져오려면
     *  토근에 sub값에 입력돼있는 사용자의 포스트만 따로 필터링 할 수 있다.
     ...
     * 
     * 6) 토큰이 만료되면 각각의 토큰을 새로 발급 받을 수 있는 엔드포인트의 요청을 해서
    ...
    /**
     * Basic ahnjnkldshfajklda
     * 
     * 1) ashndjfklashndfjkla -> email: password
    ...
    rotateToken(token: string, isRefreshToken: boolean){
        ...
        if(decoded.type !== 'refresh'){
            throw new UnauthorizedException('토큰 재발급은 Refresh토큰으로만 가능합니다!');
        }
     ...
     *  - 생성이 완료되면 accessToken과 resfreshToken을 반환한다.
     *   회원가입 후 다시 로그인해주세요 <- 이런 쓸데없는 과정을 방지하기 위해서
     * 
     * 2) loginWithEmail
     *  - email, password를 입력하면 사용자 검증을 진행한다.
     *  - 검증이 완료되면 accessToiken과 refreshToken을 반환한다
     * 
     * 3)loginUser
